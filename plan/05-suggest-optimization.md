...
6. **Prefix-result cache.** Typing appends one digit: cache `digits → Vec<idx>` (cap 8-16, keyed `(layout_id, digits, fallback-version/pack-set)` — version required once `17` synthetics land). Next keystroke filters cached set + one FST prefix page. Near-zero cost, big p95 win on long words. QWERTY path caches on `(layout,raw)`, not digits (see `22` — digit key misses on every backspace-restore cycle).
